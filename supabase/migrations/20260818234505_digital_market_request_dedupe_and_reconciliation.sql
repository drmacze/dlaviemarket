...
create index if not exists dlavie_digital_orders_reconcile_idx
  on api.dlavie_digital_orders (status, updated_at)
...
begin
  ...
  where o.wallet_id = p_wallet_id
    ...
    and o.created_at >= now() - make_interval(secs => v_seconds)
