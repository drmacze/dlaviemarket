...
export default function DLavieDigitalMarket(){
 ...
 return <main className="dlv-digital-market"><div className="dlv-digital-shell">
  ...
  <div className="dlv-digital-status"><span className={integration?.enabled?'is-on':''}/><b>{integration?.enabled?(integration.environment==='production'?'IAK aktif · Production':'IAK aktif · Testing'):'IAK catalog'}</b><em>{integration?.catalog_count||0} SKU</em><button onClick={()=>setHistoryOpen(true)}>Pesanan saya {orders.length?`· ${orders.length}`:''}</button></div>
  ...
  {!category?<section>
   ...
   <div className={`dlv-category-grid is-${view}`}>
    ...
    {!groups.length&&fallbacks.map(([name,description])=><button className="is-sync-pending" type="button" key={name} aria-disabled="true"><i><DigitalCategoryIcon value={name}/></i><span><strong>{name}</strong><small>{description} · menunggu sinkron katalog IAK</small></span><b>SYNC</b></button>)}
   </div>
   {!groups.length&&<div className="dlv-catalog-empty"><b>{kind==='postpaid'?'Katalog tagihan belum tersedia.':'Kategori sudah disiapkan.'}</b><span>{integration?.catalog_count?'Aktifkan katalog tagihan dari IAK Control lalu sinkron ulang.':'Begitu katalog IAK disinkronkan, kartu di atas otomatis berubah menjadi kategori dan produk aktif.'}</span></div>}
  </section>:
