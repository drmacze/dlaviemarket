...
const fallbackPrepaid=[
 ['Pulsa','Isi ulang semua operator'],['Paket Data','Kuota & paket internet'],['PLN','Token listrik prabayar'],['E-Wallet','Top up saldo digital'],['Voucher & Game','Voucher game dan hiburan']
] as const
const fallbackPostpaid=[['Bayar Tagihan','PLN, BPJS, PDAM, internet dan lainnya']] as const
...
export default function DLavieDigitalMarket(){
 ...
 const chooseKind=(value:MarketKind)=>{setKind(value);setCategory('');setBrand('');setQuery('');setProducts([]);setSelected(null);setReceipt(null);setTarget('');const next=value==='postpaid'?'#/market?kind=postpaid':'#/market';if(location.hash!==next)location.hash=next}
 ...
 return <main className="dlv-digital-market"><div className="dlv-digital-shell">
  ...
  {!category?<section>
   ...
   <div className={`dlv-category-grid is-${view}`}>
    ...
    {!groups.length&&fallbacks.map(([name,description])=><button className="is-sync-pending" type="button" key={name} aria-disabled="true"><i><DigitalCategoryIcon value={name}/></i><span><strong>{name}</strong><small>{description} · menunggu sinkron katalog supplier</small></span><b>SYNC</b></button>)}
   </div>
   {!groups.length&&<div className="dlv-catalog-empty"><b>{kind==='postpaid'?'Katalog tagihan belum tersedia.':'Kategori sudah disiapkan.'}</b><span>{integration?.catalog_count?'Aktifkan jenis katalog ini dari Digiflazz Control lalu sinkron ulang.':'Begitu katalog Digiflazz disinkronkan, kartu di atas otomatis berubah menjadi kategori dan produk aktif.'}</span></div>}
  </section>:
  <section className="dlv-product-stage">
   <button className="dlv-digital-back" onClick={()=>{setCategory('');setProducts([]);setSelected(null);setReceipt(null);setTarget('')}}>← Semua kategori</button>
