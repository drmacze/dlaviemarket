...
const FP_KEY='dlavie-order-evidence-fp-v2'
...
export default function DLavieOrderEvidenceBridge(){useEffect(()=>{let timer=0,busy=false
 const sync=async()=>{if(busy||sessionStorage.getItem(SESSION_KEY)!=='active')return;const p=profile(),token=localStorage.getItem(TOKEN_KEY)||'';if(!p?.id||!token)return;busy=true;try{
   ...
   if(localStorage.getItem(PROFILE_FP_KEY)!==profileFp){const accountBody=new URLSearchParams({action:'account_sync',wallet_token:token,user_id:p.id,...accountFields(p)});const ar=await fetch(API,{method:'POST',body:accountBody});if(!ar.ok)throw new Error('account_evidence_sync_failed');localStorage.setItem(PROFILE_FP_KEY,profileFp)}
