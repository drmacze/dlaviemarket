...
async function call(action:string,extra:Record<string,string>={}){
 ...
 if(!r.ok||!d.ok)throw new Error(d.message||d.error||'H2H Control gagal diproses.')
 return d
}
...
export default function DLavieH2HAdmin(){
 ...
 const load=async()=>{try{apply(await call('status'))}catch(e){setError(e instanceof Error?e.message:'Status H2H gagal dimuat.')}}
 useEffect(()=>{if(authed&&enabled)void load()},[authed,enabled])
 useEffect(()=>{if(open&&authed)void load()},[open,authed])
 const run=async(fn:()=>Promise<any>,ok:string)=>{setBusy(true);setError('');setMessage('');try{const d=await fn();setMessage(ok);await load();return d}catch(e){setError(e instanceof Error?e.message:'Operasi H2H gagal.');return null}finally{setBusy(false)}}
 ...
 return <div className={`dlv-digi-admin${open?' is-open':''}`}>
  ...
  {open&&<aside>
   <header><div><span>H2H.ID CONTROL CENTER</span><strong>Direct API · catalog · HCoin · callback</strong></div><button onClick={()=>setOpen(false)}>×</button></header>
   <div className="dlv-digi-admin-body">
    <section className="dlv-digi-health"><i className={ready?'is-on':''}/><div><strong>{ready?'H2H.id siap digunakan':'Setup H2H.id belum lengkap'}</strong><span>{data?.settings.live_enabled?'Transaksi aktif':'Transaksi dikunci'} · {data?.settings.catalog_count||0} SKU · HCoin {money.format(Number(data?.settings.last_hcoin_balance||0))} · Gateway tidak diperlukan</span></div><button disabled={busy} onClick={()=>void load()}>Refresh</button></section>
    {error&&<div className="dlv-digi-error">{error}</div>}{message&&<div className="dlv-digi-success">{message}</div>}

    <section><h3>1. Credential H2H.id</h3><p>H2H.id memakai Member ID + PIN transaksi + Password H2H. Password H2H berbeda dari password login akun. Semua nilai disimpan server-side di Supabase Vault.</p><form onSubmit={saveCred}><input value={memberId} onChange={e=>setMemberId(e.target.value)} placeholder={data?.secrets.member_id?'Member ID tersimpan · isi kembali jika mengganti credential':'Member ID / username H2H.id'} autoComplete="off"/><input type="password" inputMode="numeric" value={pin} onChange={e=>setPin(e.target.value.replace(/\D/g,'').slice(0,20))} placeholder={data?.secrets.pin?'PIN tersimpan · isi kembali jika mengganti credential':'PIN transaksi H2H'} autoComplete="new-password"/><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder={data?.secrets.password?'Password H2H tersimpan · isi kembali jika mengganti credential':'Password H2H (bukan password login)'} autoComplete="new-password"/><button disabled={busy||memberId.trim().length<2||pin.length<4||password.length<4}>Simpan credential</button></form><small>Jangan masukkan password login akun H2H. Yang dibutuhkan adalah Password H2H dari Pengaturan → API H2H.</small></section>
