...
const friendlyError=(value:unknown)=>{
 ...
 const map:Record<string,string>={
  ...
  invalid_webhook_secret:'Webhook Secret/Secret Key H2H tidak valid.',
  market_not_ready:'Setup belum lengkap. Simpan credential, Webhook Secret H2H, lalu sinkronkan katalog sebelum mengaktifkan Production.',
  hcoin_required:'HCoin belum tersedia. Top up H2H dan tekan Test koneksi & HCoin sebelum mengaktifkan Production.'
 }
...
export default function DLavieH2HAdmin(){
 ...
 const saveWebhook=async(e:FormEvent)=>{e.preventDefault();const d=await run(()=>call('save_webhook_secret',{webhook_secret:webhookKey}),'Webhook Secret H2H tersimpan. Verifikasi HMAC-SHA256 sekarang aktif.');if(d)setWebhookKey('')}
 if(!enabled||!authed)return null
 const credentials=!!data?.readiness?.credentials
 const ready=!!data?.readiness?.ready
 const production=!!data?.settings.live_enabled
 const hcoin=Number(data?.settings.last_hcoin_balance||0)
 ...
 return <div className={`dlv-digi-admin dlv-h2h-admin${open?' is-open':''}`}>
  ...
  {open&&<aside>
   <header><div><span>H2H.ID CONTROL CENTER</span><strong>Sandbox · Production · Catalog · HCoin · Webhook HMAC</strong></div><button aria-label="Tutup H2H Control Center" onClick={()=>setOpen(false)}>×</button></header>
   <div className="dlv-digi-admin-body">
    <section className="dlv-digi-health"><i className={ready?'is-on':''}/><div><strong>{ready?(production?'H2H Production aktif':'Sandbox DLavie siap diuji'):'Setup H2H.id belum lengkap'}</strong><span>{production?'PRODUCTION · memotong HCoin':'SANDBOX · tanpa HCoin/saldo asli'} · {data?.settings.catalog_count||0} SKU · HCoin {money.format(hcoin)} · HMAC {data?.secrets.webhook_key?'✓':'—'}</span></div><button disabled={busy} onClick={()=>void load()}>↻ Refresh</button></section>
    {error&&<div className="dlv-digi-error">{error}</div>}{message&&<div className="dlv-digi-success">{message}</div>}

    <section className="dlv-h2h-sandbox-admin"><h3>0. Sandbox DLavie</h3><p>Mode default untuk menguji checkout tanpa mengirim transaksi ke H2H.id. Setiap Wallet mendapat saldo virtual <b>Rp1.000.000</b>; HCoin dan saldo Wallet asli tidak dipotong.</p><div className="dlv-h2h-test-rules"><span><b>…000</b> gagal + refund</span><span><b>…999</b> tetap pending</span><span><b>lainnya</b> sukses + SN/token</span></div><small>{production?'Production sedang aktif. Matikan switch Production di bagian Harga & produk untuk kembali ke Sandbox.':'✓ Sandbox aktif sekarang. Kamu sudah bisa mencoba transaksi di Digital Market.'}</small></section>
    ...
    <section><h3>2. Koneksi langsung</h3><p>Supabase memanggil API H2H.id langsung untuk katalog, saldo HCoin, dan transaksi Production. Sandbox tidak mengirim order ke H2H. Kosongkan IP Whitelist H2H bila ingin akses direct dari Supabase.</p><div className="dlv-digi-row"><button className="is-secondary" disabled={busy||!credentials} onClick={async()=>{const d=await run(()=>call('test_connection'),'Koneksi H2H.id berhasil.');if(d?.balance!=null)setMessage(`Koneksi berhasil · HCoin ${money.format(Number(d.balance))}.`)}}>Test koneksi & HCoin</button></div>{(kyc||kyb)&&<small>Verifikasi wallet: KYC {String(kyc||'—')} · KYB {String(kyb||'—')}. Produk e-wallet tertentu dapat memerlukan verifikasi ini.</small>}</section>

    <section><h3>3. Webhook HMAC-SHA256</h3><p>Buat webhook di H2H dengan URL berikut dan centang event transaksi success, failed, pending, dan refunded. Setelah webhook dibuat, salin <b>Secret Key</b> webhook dari H2H lalu tempel di bawah. DLavie memverifikasi X-Webhook-Signature terhadap raw payload dan menolak timestamp di luar toleransi 5 menit.</p><code className="dlv-digi-url">{callback}</code><div className="dlv-digi-row"><button className="is-secondary" type="button" onClick={()=>void navigator.clipboard.writeText(callback)}>Copy URL Endpoint</button></div><form onSubmit={saveWebhook}><input type="password" value={webhookKey} onChange={e=>setWebhookKey(e.target.value)} placeholder={data?.secrets.webhook_key?'Secret Key sudah tersimpan · isi untuk mengganti':'Tempel Secret Key webhook dari H2H'} autoComplete="new-password"/><button disabled={busy||webhookKey.trim().length<16}>Simpan Webhook Secret</button></form><small>{data?.secrets.webhook_key?'✓ HMAC secret sudah tersimpan di Supabase Vault.':'Secret Key belum tersimpan. Production jangan diaktifkan sebelum HMAC siap.'}</small></section>

    <section><h3>4. Harga & produk</h3><div className="dlv-digi-grid"><label><span>Markup</span><select value={markupMode} onChange={e=>setMarkupMode(e.target.value)}><option value="fixed">Nominal Rp</option><option value="percent">Persentase %</option></select></label><label><span>Nilai markup</span><input inputMode="decimal" value={markupValue} onChange={e=>setMarkupValue(e.target.value)}/></label><label><span>Minimum markup</span><input inputMode="numeric" value={minimum} onChange={e=>setMinimum(e.target.value)}/></label></div><label className="dlv-digi-check"><input type="checkbox" checked={postpaid} onChange={e=>setPostpaid(e.target.checked)}/><span>Aktifkan katalog & flow Bayar Tagihan / PPOB</span></label><label className="dlv-digi-check"><input type="checkbox" checked={live} disabled={busy||(!live&&hcoin<=0)} onChange={e=>setLive(e.target.checked)}/><span>Gunakan H2H Production <small>— transaksi nyata, memotong HCoin</small></span></label>{!live&&hcoin<=0&&<small>Production dikunci karena HCoin masih Rp0. Sandbox tetap dapat digunakan penuh.</small>}<button disabled={busy} onClick={()=>void run(()=>call('save_settings',{markup_mode:markupMode,markup_value:markupValue,minimum_markup:minimum,postpaid_enabled:String(postpaid),live_enabled:String(live)}),live?'H2H Production diaktifkan. Transaksi berikutnya dapat memotong HCoin.':'Pengaturan disimpan. Sandbox DLavie tetap aktif.')}>Simpan pengaturan</button><small>Perubahan markup atau pilihan PPOB mereset katalog dan membutuhkan Sync ulang. Production selalu opt-in; keadaan OFF berarti Sandbox, bukan market mati.</small></section>

    <section className="dlv-digi-sync"><h3>5. Sinkron katalog H2H.id</h3><p>Katalog Pulsa, Paket Data, PLN, Voucher/Game, E-Wallet dan layanan lain disimpan ke database DLavie. Katalog asli H2H dipakai baik di Sandbox maupun Production.</p><button disabled={busy||!credentials} onClick={()=>void run(()=>call('sync_catalog'),'Katalog H2H.id selesai disinkronkan.')}>{busy?'Memproses…':'Sync katalog sekarang'}</button><small>Terakhir: {date(data?.settings.last_catalog_sync_at)} · {data?.settings.last_catalog_sync_status||'—'} · {data?.settings.last_catalog_sync_message||'—'}</small></section>
   </div>
  </aside>}
 </div>
}
