...
const friendlyError=(value:unknown)=>{
 ...
 const map:Record<string,string>={
  ...
  market_not_ready:'Setup belum lengkap. Simpan credential, Webhook Secret H2H, lalu sinkronkan katalog sebelum mengaktifkan transaksi.'
 }
...
export default function DLavieH2HAdmin(){
 ...
 return <div className={`dlv-digi-admin dlv-h2h-admin${open?' is-open':''}`}>
  ...
  {open&&<aside>
   <header><div><span>H2H.ID CONTROL CENTER</span><strong>Direct API · Catalog · HCoin · Webhook HMAC</strong></div><button aria-label="Tutup H2H Control Center" onClick={()=>setOpen(false)}>×</button></header>
   <div className="dlv-digi-admin-body">
    <section className="dlv-digi-health"><i className={ready?'is-on':''}/><div><strong>{ready?'H2H.id siap digunakan':'Setup H2H.id belum lengkap'}</strong><span>{data?.settings.live_enabled?'Transaksi aktif':'Transaksi dikunci'} · {data?.settings.catalog_count||0} SKU · HCoin {money.format(Number(data?.settings.last_hcoin_balance||0))} · HMAC {data?.secrets.webhook_key?'✓':'—'}</span></div><button disabled={busy} onClick={()=>void load()}>↻ Refresh</button></section>
